...
export default {
  ...
  methods: {
    ...
    getAreaBoxPostion(){
      ...
      var fixedwidth=0,fixedheight=0,fixedleft=0,fixedtop=0;
      visibleColumn.forEach((col,index)=>{
        if(index<startColumnIndex){
          // console.log(col)
          // console.log(this.getTablexGrid().getColumnByField(col.field).width);
          left+=this.getTablexGrid().getColumnByField(col.field).width;
        }
        if(index<=endColumnIndex&&startColumnIndex<=index){
          width+=this.getTablexGrid().getColumnByField(col.field).width;
        }
      })
    ...
    tbodykeydown(event){
      ...
      if (event.ctrlKey && event.keyCode === 67) {
        ...
        console.log(tableData=this.getTablexGrid().getTableData()["visibleData"])
        event.preventDefault();
